...
define(["../buildControl"], function(bc) {
	...
	return function(resource) {
		...
		if (bc.stripConsole) {
			text= text.replace(/([^\w\.])(console)(\.(assert|count|debug|dir|dirxml|error|group|groupEnd|info|profile|profileEnd|time|timeEnd|race|warn|log)\()/g, function(match, prefix, console, suffix) {
				return prefix + " false && " + console + suffix;
			});
		}
