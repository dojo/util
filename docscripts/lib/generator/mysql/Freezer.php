...
final class Freezer
{
  ...
  public function open($key, $default) {
    $query = db_query("SELECT value FROM freezer WHERE namespace = '%s' AND id = '%s'", $this->namespace, $key);
  ...
  public function save($key, $content) {
    ...
    $query = db_query("SELECT 1 FROM freezer WHERE namespace = '%s' AND id = '%s'", $this->namespace, $key);
    if (db_fetch_assoc($query)) {
      db_query("UPDATE freezer SET value = '%s' WHERE namespace = '%s' AND id = '%s'", $content, $this->namespace, $key);
    }
    else {
